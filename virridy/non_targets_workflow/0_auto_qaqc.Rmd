...
thresholds. Lastly, we run the pipeline across all data.*

To run this pipeline, you must have the following additional data sets:

1) Field notes
2) A yaml containing your HydroVu credentials
3) A yaml containing your mWater credentials
...
lapply(c("data.table", "tidyverse", "rvest", "readxl", "lubridate", "zoo", "padr", "stats",
...
old_field_notes <- load_old_field_notes(filepath = "data/sensor_field_notes.xlsx",
                                        # what interval to round raw data to:
...
sensor_malfunction_notes <- load_mWater_notes(creds = yaml::read_yaml("creds/mWaterCreds.yml"), 
                                              # what interval to round raw data to:
                                              summarize_interval = "15 minutes") %>%
...
hv_token <- HydroVuR::hv_auth(client_id = as.character(hv_creds["client"]),
                    client_secret = as.character(hv_creds["secret"]),
                    url = "https://www.hydrovu.com/public-api/oauth/token")

# Pull data for ALL sites, starting in 2023 
...
#   "Prospect", #"Prospect virridy", 
#   "Boxelder",
#   #"BoxCreek", 
#   "Archery", #"Archery virridy",
#   "River Bluffs"),
#   network = "csu",
#   start_dt = "2024-07-29 01:00:00 MDT",#"2023-01-01 01:00:00 MDT",
#   end_dt = "2024-07-29 03:00:00 MDT", #Sys.time(),
#   api_token = hv_token,
#   dump_dir = "data/api/fcw_example/" #/all_sites/"
# )

hacky <- TRUE

if(hacky == FALSE){ # TODO: figure out when to use these two functions
...
all_data_tidied_list <- purrr::map2(.x = site_param_combos$sites, 
                                    ...
                                    ~tidy_and_add_field_notes(site_arg = .x,
                                                              parameter_arg = .y,
                                                              api_data = all_data,
                                                              # should be the same interval as what was selected for
                                                              # all upstream steps
